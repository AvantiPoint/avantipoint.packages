...
public class CertificateDownloadApiTests : IDisposable
{
    ...
    public async Task GetCertificate_WithValidFingerprint_ReturnsCertificateFile()
    {
        ...
        // Act
        var response = await client.GetAsync($"/v3/certificates/{fingerprint}.crt", TestContext.Current.CancellationToken);

        // Assert
        response.EnsureSuccessStatusCode();
        Assert.Equal("application/x-x509-ca-cert", response.Content.Headers.ContentType?.MediaType);
        
        var certificateBytes = await response.Content.ReadAsByteArrayAsync(TestContext.Current.CancellationToken);
    ...
    public async Task GetCertificate_WithNonExistentFingerprint_Returns404()
    {
        ...
        // Act
        var response = await client.GetAsync($"/v3/certificates/{nonExistentFingerprint}.crt", TestContext.Current.CancellationToken);
    ...
    public async Task GetCertificate_WithEmptyFingerprint_Returns400()
    {
        ...
        // Act
        var response = await client.GetAsync("/v3/certificates/.crt", TestContext.Current.CancellationToken);
    ...
    public async Task GetCertificate_OnlyReturnsSha256Certificates()
    {
        ...
        var sha384Response = await client.GetAsync($"/v3/certificates/{sha384Fingerprint}.crt", TestContext.Current.CancellationToken);

        // Assert
        Assert.Equal(HttpStatusCode.NotFound, sha384Response.StatusCode);

        // Act - Try to get SHA-256 fingerprint (should succeed)
        var sha256Response = await client.GetAsync($"/v3/certificates/{sha256Fingerprint}.crt", TestContext.Current.CancellationToken);
