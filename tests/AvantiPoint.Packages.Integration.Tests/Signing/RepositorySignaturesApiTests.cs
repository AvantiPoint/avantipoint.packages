...
public class RepositorySignaturesApiTests : IDisposable
{
        ...
        return new WebApplicationFactory<IntegrationTestApi.Program>()
            .WithWebHostBuilder(builder =>
            {
                builder.ConfigureTestServices(services =>
                {
                    ...
                    // Replace RepositorySigningCertificateService to use scoped IContext
                    var serviceDescriptor = services.FirstOrDefault(s => s.ServiceType == typeof(RepositorySigningCertificateService));
                    if (serviceDescriptor != null)
                    {
                        services.Remove(serviceDescriptor);
                    }
                    services.AddScoped<RepositorySigningCertificateService>(provider =>
    ...
    public async Task GetRepositorySignatures_WithNoCertificates_ReturnsEmptyList()
    {
        ...
        // Act
        var response = await client.GetAsync("/v3/repository-signatures/index.json", TestContext.Current.CancellationToken);

        // Assert
        response.EnsureSuccessStatusCode();
        var content = await response.Content.ReadFromJsonAsync<RepositorySignaturesResponse>(TestContext.Current.CancellationToken);
    ...
    public async Task GetRepositorySignatures_WithActiveCertificate_ReturnsCertificate()
    {
        ...
        await service.RecordCertificateUsageAsync(certificate, TestContext.Current.CancellationToken);
    ...
    public async Task GetRepositorySignatures_ExcludesInactiveCertificates()
    {
        ...
        await service.RecordCertificateUsageAsync(cert1, TestContext.Current.CancellationToken);
        await service.RecordCertificateUsageAsync(cert2, TestContext.Current.CancellationToken);

        var fingerprint2 = TestCertificateHelper.ComputeSha256Fingerprint(cert2);
        await service.DeactivateCertificateAsync(fingerprint2, CertificateHashAlgorithm.Sha256, "Test deactivation", TestContext.Current.CancellationToken);
    ...
    public async Task GetRepositorySignatures_ExcludesExpiredCertificatesNotRecentlyUsed()
    {
        ...
        await service.RecordCertificateUsageAsync(expiredCert, TestContext.Current.CancellationToken);

        // Set LastUsed to 100 days ago (outside 90-day grace period)
        var saved = await _context.RepositorySigningCertificates
            .FirstAsync(c => c.Subject == expiredCert.Subject, TestContext.Current.CancellationToken);
    ...
    public async Task GetRepositorySignatures_WithMultipleCertificates_OrdersByFirstUsedDescending()
    {
        ...
        await service.RecordCertificateUsageAsync(cert3, TestContext.Current.CancellationToken);
    ...
    public async Task GetRepositorySignatures_WithNullContentUrl_ExcludesContentUrlFromJson()
    {
        ...
        // Clear ContentUrl
        var saved = await _context.RepositorySigningCertificates.FirstAsync(TestContext.Current.CancellationToken);
