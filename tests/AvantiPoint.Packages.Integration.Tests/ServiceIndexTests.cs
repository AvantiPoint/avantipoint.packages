...
using AvantiPoint.Packages.Integration.Tests.Fixtures;
...
public class ServiceIndexTests : IClassFixture<ServiceIndexTestFixture>, IDisposable
{
    ...
    public async Task ServiceIndex_IncludesAllRequiredEndpoints()
    {
        ...
        // Act
        var response = await client.GetAsync("/v3/index.json", TestContext.Current.CancellationToken);
        
        // Assert
        response.EnsureSuccessStatusCode();
        var serviceIndex = await response.Content.ReadFromJsonAsync<ServiceIndexResponse>(TestContext.Current.CancellationToken);
    ...
    public async Task VulnerabilityIndex_IsAccessible()
    {
        ...
        var indexResponse = await client.GetAsync("/v3/index.json", TestContext.Current.CancellationToken);
        indexResponse.EnsureSuccessStatusCode();
        var serviceIndex = await indexResponse.Content.ReadFromJsonAsync<ServiceIndexResponse>(TestContext.Current.CancellationToken);
        ...
        var vulnerabilityResponse = await client.GetAsync("/v3/vulnerabilities/index.json", TestContext.Current.CancellationToken);

        // Assert
        vulnerabilityResponse.EnsureSuccessStatusCode();
        var content = await vulnerabilityResponse.Content.ReadAsStringAsync(TestContext.Current.CancellationToken);
    ...
    public async Task VulnerabilityIndex_ReturnsValidJson_WhenEnabled()
    {
        ...
        // Act
        var response = await client.GetAsync("/v3/vulnerabilities/index.json", TestContext.Current.CancellationToken);

        // Assert
        response.EnsureSuccessStatusCode();
        var content = await response.Content.ReadFromJsonAsync<VulnerabilityIndexResponse>(TestContext.Current.CancellationToken);
