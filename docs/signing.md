...
Repository signing is automatically enabled when you configure it. No additional code is required:
...
builder.Services.AddNuGetPackageApi(options =>
{
    ...
    // Signing is automatically configured from appsettings.json
});
```
...
1. **Use stored certificates or cloud key vaults in production** - Self-signed certificates are suitable for development/testing only
2. **Enable timestamping** - Ensures signatures remain valid after certificate expiration
3. **Store passwords and secrets securely** - Use configuration keys with environment variables or secret stores
...
7. **Use cloud key vaults for HSM-backed keys** - Azure Key Vault, AWS KMS, and GCP KMS provide FIPS 140-2 Level 2/3 validated HSMs
8. **Note on AWS and GCP providers** - AWS KMS/Signer and GCP KMS/HSM currently require custom signing implementations as they don't export private keys
...
- [Repository Signatures API](../docs-site/docs/api-repository-signatures.md) - API endpoint documentation
