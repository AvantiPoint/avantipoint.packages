...
```markdown
![Package Version](https://your-feed.example.com/shield/MyPackage.svg)
```
...
```markdown
## MyPackage ![Version](https://your-feed.example.com/shield/MyPackage.svg)

Current version: ![MyPackage Version](https://your-feed.example.com/shield/MyPackage.svg)
```
...
```markdown
### Newtonsoft.Json
![Version](https://your-feed.example.com/shield/Newtonsoft.Json.svg)
```
...
| Package | Version |
|---------|---------|
| MyCompany.Core | ![Version](https://your-feed.example.com/shield/MyCompany.Core.svg) |
| MyCompany.Web | ![Version](https://your-feed.example.com/shield/MyCompany.Web.svg) |
| MyCompany.Data | ![Version](https://your-feed.example.com/shield/MyCompany.Data.svg) |
```
...
![Package Version](https://nuget.mycompany.com/shield/MyCompany.MyPackage.svg)
...
```markdown
![Private Feed](https://your-feed.example.com/shield/MyCompany.Package.svg)
```
...
3. **Check URL** - Ensure the URL matches `shield/{id}` or `shield/{id}/vpre` (case-sensitive on Linux)

### Stable vs Prerelease

- Use `shield/{packageId}.svg` for the latest stable listed version.
- Use `shield/{packageId}/vpre.svg` to include prerelease versions (e.g. `2.0.0-beta.4`).
- If only prerelease versions exist, the stable badge may be empty or show the latest prerelease depending on listing configuration.
...
3. **Add cache-busting parameter** - Add a query parameter to force refresh:
   ```markdown
  ![Version](https://your-feed.example.com/shield/MyPackage.svg?v=2)
   ```
